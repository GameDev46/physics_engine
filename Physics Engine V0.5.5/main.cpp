...
class warningPanel {
    public:
        ...
        void displayMessage(string title, string msg, string acceptButtonText, string denyButtonText, int focusID) {
            if (focus != 0) return;
            ...
            butt1Text = denyButtonText;
            butt2Text = acceptButtonText;
        }

        int update(Vector2 mousePosition, bool isMouseDown) {

            if (focus != 0) {

                // Background

                Vector2 centerPos = Vector2({ GetScreenWidth() / 2.0f, GetScreenHeight() / 2.0f });
                Vector2 size = Vector2({ 300.0f, 200.0f });

                // Outline
                Rectangle rect = { centerPos.x - (size.x * 0.5f) - 2.0f, centerPos.y - (size.y * 0.5f) - 2.0f, size.x + 4.0f, size.y + 4.0f };
                DrawRectangleRounded(rect, 0.05f, 10, GetColor(0x0b101dff));

                // Panel Backing
                rect = { centerPos.x - (size.x * 0.5f), centerPos.y - (size.y * 0.5f), size.x, size.y };
                DrawRectangleRounded(rect, 0.05f, 10, GetColor(0x1b202dff));

                // Title outline
                rect = { centerPos.x - (size.x * 0.5f), centerPos.y - (size.y * 0.5f), size.x, (size.y / 4.0f) + 2.0f };
                DrawRectangleRounded(rect, 0.05f, 10, GetColor(0x0b101dff));

                // Title backing
                rect = { centerPos.x - (size.x * 0.5f), centerPos.y - (size.y * 0.5f), size.x, size.y / 4.0f };
                DrawRectangleRounded(rect, 0.05f, 10, GetColor(0x1f2432ff));

                // Panel title
                DrawText(currentTitle.data(), centerPos.x - (size.x * 0.5f) + 20.0f, centerPos.y - (size.y * 0.5f) + 15.0f, 20, GetColor(0xffffffff) );

                // Description
                DrawText(currentMsg.data(), centerPos.x - (size.x * 0.5f) + 10.0f, centerPos.y - (size.y * 0.5f) + (size.y / 4.0f) + 30.0f, 12, GetColor(0xffffffff) );

                if (butt1Text == butt2Text) {

                    // Accept button

                    const char *button2CharArray = butt2Text.data();
                    for (int i = 0; i < 256; i++) {
                        acceptButton.buttonText[i] = button2CharArray[i];
                    }

                    acceptButton.position = Vector2({ centerPos.x - (size.x * 0.5f) + 10.0f, centerPos.y + (size.y * 0.5f) - 60.0f });
                    acceptButton.size = Vector2({ size.x - 20.0f, 50.0f });
                    acceptButton.colour = GetColor(0xe85b4aff);
                    acceptButton.hoverColour = GetColor(0xcf4838ff);

                    if (acceptButton.isMouseOverButton(mousePosition) && isMouseDown) {
                        // Accept action
                        int currentFoc = focus;
                        focus = 0;
                        return currentFoc;
                    }

                    acceptButton.draw(mousePosition);
                    ...
                    acceptButton.position = Vector2({ centerPos.x + 10.0f, centerPos.y + (size.y * 0.5f) - 60.0f });
                    acceptButton.size = Vector2({ (size.x * 0.5f) - 20.0f, 50.0f });
                    acceptButton.colour = GetColor(0x24a652ff);
                    acceptButton.hoverColour = GetColor(0x1bc457ff);
                    ...
                    const char *button1CharArray = butt1Text.data();
                    for (int i = 0; i < 256; i++) {
                        closeButton.buttonText[i] = button1CharArray[i];
                    }

                    closeButton.position = Vector2({ centerPos.x - (size.x * 0.5f) + 10.0f, centerPos.y + (size.y * 0.5f) - 60.0f });
                    closeButton.size = Vector2({ (size.x * 0.5f) - 20.0f, 50.0f });
                    closeButton.colour = GetColor(0xe85b4aff);
                    closeButton.hoverColour = GetColor(0xcf4838ff);

                    if (closeButton.isMouseOverButton(mousePosition) && isMouseDown) {
                        // Accept action
                        focus = 0;
                        return 0;
                    }
...
bool saveFile(string fileName, circleCollider* circlesData, int maxCircles, SpringJoint* springsData, int maxSprings, objectBounder worldBound, float worldGravitySetting, bool useMoment, bool rewriteFile) {
...
bool loadFile(string fileName, circleCollider* circlesData, int maxCircles, SpringJoint* springsData, int maxSprings, objectBounder *worldBound, float *worldGravitySetting, bool *useMoment) {
...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        snprintf(systemTotalEnergyText, sizeof(systemTotalEnergyText), "System Energy: %d MJ", int(round(systemTotalEnergy / 1000000.0f)) );
        ...
        if (saveButton.isMouseOverButton(mousePosition) && isMouseDown && !wasMouseDown) {
            ...
            // Save simulation
            bool success = saveFile(str, sceneCircleColliders, sceneCircleCount, sceneSpringJoints, sceneSpringCount, worldBounding, worldGravity, useMomentum, false);

            if (!success) {
                informationPanel.displayMessage("File Already Exists", "The file '" + str + ".txt' already exists!\n Would you like to rewrite its contents?", "Rewrite", "Cancel", 2);
            }
        ...
        if (loadButton.isMouseOverButton(mousePosition) && isMouseDown && !wasMouseDown) {
            ...
            // Load Simulation
            bool success = loadFile(str, sceneCircleColliders, sceneCircleCount, sceneSpringJoints, sceneSpringCount, &worldBounding, &worldGravity, &useMomentum);
            ...
            if (success) {
                ...
                informationPanel.displayMessage("Failed To Load", "The file '" + str + ".txt' was not found! \nPlease check if the name is typed in correctly.", "Dismiss", "Dismiss", 1);
            }
        ...
        int nextStepFocusID = informationPanel.update(mousePosition, isMouseDown && !wasMouseDown);

        if (nextStepFocusID > 0) {
            // Accepted

            // Overwrite save file
            if (nextStepFocusID == 2) {
                string str(lastSaveFile);
                saveFile(str, sceneCircleColliders, sceneCircleCount, sceneSpringJoints, sceneSpringCount, worldBounding, worldGravity, useMomentum, true);
            }
